    ...
    startingDateAndTime: {
      ...
      validate: {
        validator: (val) => {
          ...
          const normalizedStartDate = moment(val).tz('America/Toronto').toISOString();
...
RequestSchema.virtual('isHappeningToday').get(function () {
  const startOfDay = moment().tz('America/Toronto').startOf('day').toISOString();
  const endOfDay = moment().tz('America/Toronto').endOf('day').toISOString();
